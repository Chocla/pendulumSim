...
class cart():
    ...
    def __init__(self, state, m=1., M=5., L=50.):
        self.state = state
        self.m = m
        self.M = M
        self.L = L
        ...
        self.K = calculateK(self.m,self.M,self.L,9.8,self.d,self.eigs)
        # print(self.K)
        # self.K = np.zeros(4)
    ...
    def update(self):
        #FIXME: Eigenvalue-placed K vector doesn't actually stabilize system
        self.state = heuns_method_cart(0.01,self.state,self.m,self.M,self.L,9.8,self.d,
        -1*np.dot(self.K,(self.state-[1,0,0,0]))) 
        self.x = self.state[0]
